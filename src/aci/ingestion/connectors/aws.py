...
class AWSBillingConnector:
    ...
    def transform_cur_record(self, record: dict[str, Any]) -> DomainEvent:
        ...
        billing = BillingLineItem(
            ...
            cost_usd=_coerce_float(raw_cost, field_name="lineItem/UnblendedCost"),
            usage_quantity=_coerce_float(
                record.get("lineItem/UsageAmount", 0),
                field_name="lineItem/UsageAmount",
            ),
...
class BedrockTelemetryConnector:
    ...
    def transform_invocation(self, log_entry: dict[str, Any]) -> DomainEvent:
        ...
        request_id = str(log_entry.get("requestId", "")).strip()
        if not request_id:
            request_id = f"bedrock-fallback:{self._stable_invocation_fingerprint(log_entry)}"
        inference = InferenceEvent(
            ...
            input_tokens=_coerce_int(
                log_entry.get("inputTokenCount", 0),
                field_name="inputTokenCount",
            ),
            output_tokens=_coerce_int(
                log_entry.get("outputTokenCount", 0),
                field_name="outputTokenCount",
            ),
            latency_ms=_coerce_float(
                log_entry.get("invocationLatency", 0),
                field_name="invocationLatency",
            ),
    ...
    def _estimate_cost(log_entry: dict[str, Any]) -> float:
        ...
        input_tokens = _coerce_float(
            log_entry.get("inputTokenCount", 0),
            field_name="inputTokenCount",
        )
        output_tokens = _coerce_float(
            log_entry.get("outputTokenCount", 0),
            field_name="outputTokenCount",
        )
    ...
    @staticmethod
    def _stable_invocation_fingerprint(log_entry: dict[str, Any]) -> str:
        material = {
            "model_id": str(log_entry.get("modelId", "")),
            "model_arn": str(log_entry.get("modelArn", "")),
            "region": str(log_entry.get("region", "")),
            "timestamp": str(log_entry.get("timestamp", "")),
            "input_tokens": str(log_entry.get("inputTokenCount", "")),
            "output_tokens": str(log_entry.get("outputTokenCount", "")),
        }
        payload = json.dumps(material, sort_keys=True, separators=(",", ":")).encode("utf-8")
        return hashlib.sha256(payload).hexdigest()[:24]
