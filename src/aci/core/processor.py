...
class AttributionProcessor:
    ...
    def _handle_inference(self, event: DomainEvent) -> None:
        ...
        logger.info(
            ...
            correlation_id=str(event.attributes.get("correlation_id", "")),
        )
    ...
    def _handle_correction(self, event: DomainEvent) -> None:
        ...
        self._processed_count += 1
        logger.info(
            "processor.correction_processed",
            attribution_id=label.attribution_id,
            correlation_id=str(event.attributes.get("correlation_id", "")),
        )
    ...
    def _build_context(
        ...
        # Look for edges from the workload to known entities. Preserve one
        # deterministic direct target when it exists; otherwise retain all
        # weighted targets so the proportional allocator can reason about
        ...
        for lookup_id in self._candidate_graph_node_ids(workload_id):
            ...
            for edge in edges:
                ...
                if target and target.node_type in (NodeType.TEAM, NodeType.PERSON):
                    direct_targets[target.node_id] = max(
        ...
        if len(direct_targets) == 1:
            ...
            ctx.proportional_users.update(direct_targets)
