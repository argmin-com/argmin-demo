...
def validate_event_attributes(
    ...
    *,
    ...
    _validate_json_shape(attributes, max_depth=max_depth, max_keys=max_keys)
...
def _validate_json_shape(
    ...
    *,
    ...
    """Reject deeply nested or excessively wide event payloads before schema validation."""
    ...
    def walk(node: object, *, depth: int) -> None:
        ...
        if depth > max_depth:
            raise EventSchemaValidationError(
                f"Event payload exceeds max depth {max_depth}"
            )
        ...
        if isinstance(node, dict):
            ...
            if key_count > max_keys:
                raise EventSchemaValidationError(
                    f"Event payload exceeds max key count {max_keys}"
                )
