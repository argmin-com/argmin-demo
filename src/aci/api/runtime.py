...
class DisabledInterceptor:
    ...
    def stats(self) -> dict[str, object]:
        return {
            ...
            "fail_open_window_rate": 0.0,
            "fail_open_alert_level": "normal",
        }
...
class AppState:
    ...
    def __init__(self) -> None:
        ...
        self.pricing = PricingCatalog.with_default_rules(
            ...
            redis_prefix=self.config.pricing_redis_prefix,
        )
        self.reconciliation = ReconciliationLedger(
            redis_url=(
                self.config.redis_url if self.config.reconciliation_backend == "redis" else None
            ),
            redis_prefix=self.config.reconciliation_redis_prefix,
        )
        self.adoption = AdoptionAnalytics(
            retention_days=self.config.adoption.retention_days,
            repeat_user_request_threshold=self.config.adoption.repeat_user_request_threshold,
            power_user_request_threshold=self.config.adoption.power_user_request_threshold,
            redis_url=self.config.redis_url if self.config.adoption.backend == "redis" else None,
            redis_prefix=self.config.adoption.redis_prefix,
        )
        self.forecast = SpendForecastEngine()
        self.notification_hub = NotificationHub(
            live_network=self.config.notifications.live_network,
            timeout_seconds=self.config.notifications.timeout_seconds,
            retry_attempts=self.config.notifications.retry_attempts,
            retry_backoff_seconds=self.config.notifications.retry_backoff_seconds,
            webhook_allowlist=self.config.notifications.webhook_allowlist.split(","),
            allow_private_targets=self.config.notifications.allow_private_targets,
            history_max_entries=self.config.notifications.history_max_entries,
        )
        self.cost_simulation = CostSimulationEngine(self.pricing)
        self.intervention_registry = InterventionRegistry.with_seed_data(
            redis_url=(
                self.config.redis_url if self.config.interventions_backend == "redis" else None
            ),
            redis_prefix=self.config.interventions_redis_prefix,
        )
        ...
        if self.accepts_ingestion:
            ...
            self.event_bus.subscribe(
                EventType.INFERENCE_REQUEST.value,
                self.adoption.handle_inference_request,
            )
            self.event_bus.subscribe(
                EventType.IDENTITY_LOGIN.value,
                self.adoption.record_identity_login,
            )
            self.event_bus.subscribe(
                EventType.ORG_CHANGE.value,
                self.adoption.handle_org_change,
            )
            self.event_bus.subscribe(
                EventType.TEAM_MEMBERSHIP.value,
                self.adoption.handle_team_membership,
            )
        else:
            self.event_bus = (
    ...
    async def stop(self) -> None:
        ...
        await self.notification_hub.aclose()
        self.intervention_registry.close()
    ...
    def readiness_checks(self) -> dict[str, bool]:
        ...
        checks["pricing_backend_healthy"] = self.pricing.durable_backend_healthy()
        checks["reconciliation_backend_healthy"] = self.reconciliation.durable_backend_healthy()
        checks["interventions_backend_healthy"] = (
            self.intervention_registry.durable_backend_healthy()
        )
        checks["adoption_backend_healthy"] = self.adoption.durable_backend_healthy()
