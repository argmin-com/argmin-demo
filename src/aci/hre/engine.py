...
class HeuristicReconciliationEngine:
    ...
    def resolve(
        ...
        cache_key = self._build_cache_key(entity_id, entity_type, event_time, context)
    ...
    def _build_cache_key(
        ...
        payload = {
            ...
            "identity_mappings": sorted(context.identity_mappings.items()),
            "temporal_events": [
                [cls._normalize_datetime(ts), other_entity, source]
                for ts, other_entity, source in context.temporal_events
            ],
            "naming_patterns": {
                team_id: list(patterns)
                for team_id, patterns in sorted(context.naming_patterns.items())
            },
            "historical_attributions": [
                [team_id, confidence]
                for team_id, confidence in context.historical_attributions
            ],
            "deployment_owners": [
                [owner, cls._normalize_datetime(ts)] for owner, ts in context.deployment_owners
            ],
            "code_owners": list(context.code_owners),
            "recent_users": [
                [user, cls._normalize_datetime(ts)] for user, ts in context.recent_users
            ],
            "proportional_users": sorted(context.proportional_users.items()),
        }
        fingerprint = hashlib.sha256(
            json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest()
        return f"{context.tenant_id}:{entity_type}:{entity_id}:{fingerprint}"
