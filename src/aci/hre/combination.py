...
def combine_evidence(
    ...
    indexed_signals = [
        (idx, method, confidence)
        for idx, (method, confidence) in enumerate(bounded_signals)
    ]
    individual: list[IndividualEvidence] = []
    penalties: list[DependencyPenalty] = []
    weights: dict[int, float] = {idx: 1.0 for idx, _method, _confidence in indexed_signals}

    for i, (_idx_a, method_a, confidence_a) in enumerate(indexed_signals):
        for j, (_idx_b, method_b, confidence_b) in enumerate(indexed_signals):
            if j <= i:
                continue
            ...
            if pair in correlations:
                discount = correlations[pair]
            elif pair_rev in correlations:
                discount = correlations[pair_rev]
            else:
                discount = None
            if discount is not None:
                # Apply dependency discount to the weaker signal.
                if confidence_a <= confidence_b:
                    weights[indexed_signals[i][0]] = min(weights[indexed_signals[i][0]], discount)
                else:
                    weights[indexed_signals[j][0]] = min(weights[indexed_signals[j][0]], discount)
    ...
    sorted_signals = sorted(indexed_signals, key=lambda x: x[2], reverse=True)
    ...
    product_term = 1.0
    for idx, (signal_idx, method, conf) in enumerate(sorted_signals):
        w = weights.get(signal_idx, 1.0)
