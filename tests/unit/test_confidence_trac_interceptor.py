...
from aci.config import InterceptorConfig, TRACConfig
...
class TestTRACCalculator:
    ...
    def test_patent_sensitivity_note(self) -> None:
        ...
        assert result.carbon_liability_usd == 0.0
        assert result.risk_pct_of_trac > 0.0

    def test_trac_can_include_carbon_when_enabled(self) -> None:
        trac = TRACCalculator(TRACConfig(include_carbon_liability=True))
        result = trac.compute(
            workload_id="carbon-enabled",
            billed_cost_usd=100.0,
            emissions_kg_co2e=1.0,
            attribution_confidence=0.70,
        )
...
class TestFailOpenInterceptor:
    ...
    async def test_advisory_enrichment(self) -> None:
        ...
        assert result.enrichment_headers["X-ACI-Cost-Center-Id"] == "CC-1200"
    ...
    @pytest.mark.asyncio
    async def test_header_budget_retains_critical_headers(self) -> None:
        entry = self._make_index_entry("svc-header-budget").model_copy(
            update={
        ...
        interceptor = FailOpenInterceptor(
            self.store,
            config=InterceptorConfig(
                max_enrichment_header_bytes=220,
                max_header_value_length=64,
            ),
            mode=DeploymentMode.ADVISORY,
        )

        result = await interceptor.intercept(self._make_request(service_name="svc-header-budget"))

        assert result.enrichment_headers["X-ACI-Attribution-Owner-Id"] == "team-cs"
        assert result.enrichment_headers["X-ACI-Cost-Center-Id"] == "CC-1200"
        assert "X-ACI-Team-Name" not in result.enrichment_headers
        assert result.enrichment_headers["X-ACI-Header-Budget-Applied"] == "1"

    @pytest.mark.asyncio
    async def test_fail_open_alert_level_tracks_rolling_window(self) -> None:
        self.store.materialize(self._make_index_entry("svc-alert-window"))
        interceptor = FailOpenInterceptor(
            self.store,
            config=InterceptorConfig(
                fail_open_alert_window_requests=4,
                fail_open_warn_rate=0.25,
                fail_open_critical_rate=0.5,
            ),
            mode=DeploymentMode.ADVISORY,
        )

        await interceptor.intercept(self._make_request(service_name="svc-alert-window"))
        await interceptor.intercept(self._make_request(service_name="unknown-a"))
        await interceptor.intercept(self._make_request(service_name="unknown-b"))
        await interceptor.intercept(self._make_request(service_name="svc-alert-window"))

        metrics = interceptor.stats
        assert metrics["fail_open_alert_level"] == "critical"
        assert metrics["fail_open_window_rate"] == pytest.approx(0.5)
...
async def test_policy_evaluation_emits_audit_event() -> None:
    ...
    assert policy_event.attributes["correlation_id"] == ""
    assert policy_event.attributes["violation_count"] == 1
    assert policy_event.attributes["violated_policy_ids"] == ["model_allowlist"]


@pytest.mark.asyncio
async def test_policy_and_intervention_events_propagate_correlation_id() -> None:
    store = AttributionIndexStore()
    store.materialize(
        AttributionIndexEntry(
            workload_id="svc-corr-route",
            team_id="team-opt",
            team_name="Optimization",
            cost_center_id="CC-3300",
            confidence=0.96,
    ...
    request = InterceptionRequest(
        ...
        model="gpt-4o",
        service_name="svc-corr-route",
        estimated_cost_usd=0.0042,
        metadata={"environment": "staging", "correlation_id": "corr-123"},
    )
    ...
    assert len(bus.events) == 2
    assert {event.attributes["correlation_id"] for event in bus.events} == {"corr-123"}
