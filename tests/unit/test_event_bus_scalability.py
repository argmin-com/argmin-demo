...
@pytest.mark.asyncio
async def test_inmemory_publish_raises_when_a_handler_fails() -> None:
    bus = InMemoryEventBus(max_events=16)
    ...
    with pytest.raises(RuntimeError, match="boom"):
        await bus.publish(make_event("evt-handler-fail", "key-handler-fail"))

    assert seen_async == ["evt-handler-fail"]
    assert cast("int", bus.stats["dispatch_errors"]) == 1
