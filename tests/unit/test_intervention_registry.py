...
def test_registry_transitions_and_summary_metrics() -> None:
    ...
    assert updated.history[0].from_status == "recommended"
    assert updated.history[0].to_status == "approved"
...
def test_registry_enforces_transition_rules_and_audit_notes() -> None:
    registry = InterventionRegistry.with_seed_data()
...
def test_intervention_lifecycle_api_endpoints() -> None:
    ...
    try:
        with TestClient(app) as client:
            ...
            implemented_payload = implemented.json()
            assert implemented_payload["status"] == "implemented"
            assert implemented_payload["updated_by"] == "unit-test"

            history = client.get("/v1/interventions/INT-401/history")
            assert history.status_code == 200
            history_payload = history.json()
            assert len(history_payload) == 2
            assert history_payload[0]["to_status"] == "approved"
            assert history_payload[1]["to_status"] == "implemented"
