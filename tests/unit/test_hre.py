...
class TestEvidenceCombination:
    ...
    def test_zero_discount_value_is_not_overridden_by_reverse_lookup(self) -> None:
        config = CombinationConfig(
            correlated_pairs={
                ("R2", "R3"): 0.0,
                ("R3", "R2"): 0.5,
        ...
        result = combine_evidence([("R2", 0.6), ("R3", 0.5)], config)
        assert abs(result.combined_confidence - 0.6) < 0.001

    def test_duplicate_method_signals_keep_independent_weights(self) -> None:
        config = CombinationConfig(correlated_pairs={("R3", "R4"): 0.2})
        result = combine_evidence([("R3", 0.3), ("R3", 0.7), ("R4", 0.4)], config)

        weighted_by_signal = [
            (entry.method, entry.raw_confidence, entry.weight)
            for entry in result.individual_confidences
        ]
        assert ("R3", 0.7, 1.0) in weighted_by_signal
        assert ("R3", 0.3, 0.1) in weighted_by_signal
        assert ("R4", 0.4, 0.2) in weighted_by_signal
...
class TestHREOrchestrator:
    ...
    def test_cache_key_includes_context_and_event_time(self) -> None:
        now = datetime.now(UTC)

        ctx_a = ReconciliationContext()
        ctx_a.temporal_events = [(now - timedelta(seconds=5), "team-a", "cloudtrail")]

        ctx_b = ReconciliationContext()
        ctx_b.temporal_events = [(now - timedelta(seconds=5), "team-b", "cloudtrail")]

        result_a = self.hre.resolve(
            entity_id="endpoint-cache-sensitive",
            entity_type="cloud_resource",
            event_time=now,
            context=ctx_a,
        )
        result_b = self.hre.resolve(
            entity_id="endpoint-cache-sensitive",
            entity_type="cloud_resource",
        ...
        assert result_a.explanation.target_entity == "team-a"
        assert result_b.explanation.target_entity == "team-b"
