...
def test_validate_event_attributes_enforces_max_depth() -> None:
    with pytest.raises(EventSchemaValidationError, match="max depth"):
        validate_event_attributes(
            EventType.RESOURCE_CREATED,
            {
                "resource_arn": "arn:aws:sagemaker:us-east-1:123:endpoint/example",
...
def test_validate_event_attributes_enforces_max_key_count() -> None:
    with pytest.raises(EventSchemaValidationError, match="max key count"):
        validate_event_attributes(
            EventType.RESOURCE_CREATED,
            {
                "resource_arn": "arn:aws:sagemaker:us-east-1:123:endpoint/example",
