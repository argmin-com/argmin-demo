...
def test_aws_cur_rejects_invalid_numeric_fields() -> None:
    connector = AWSBillingConnector(tenant_id="tenant-a")
    with pytest.raises(ValueError, match="lineItem/UnblendedCost"):
        connector.transform_cur_record(
            {
                "lineItem/UsageAccountId": "123456789012",
                "lineItem/ProductCode": "AmazonBedrock",
...
def test_bedrock_connector_uses_fallback_request_id_and_numeric_coercion() -> None:
    connector = BedrockTelemetryConnector(tenant_id="tenant-a")
    event = connector.transform_invocation(
        {
            ...
            "invocationLatency": "38.2",
            "timestamp": "2026-03-05T03:20:00Z",
        }
    )

    assert event.idempotency_key.startswith("bedrock:bedrock-fallback:")
    assert event.event_type == EventType.INFERENCE_REQUEST
    assert event.attributes["input_tokens"] == 1200
    assert event.attributes["output_tokens"] == 450
