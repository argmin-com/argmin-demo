...
class TestFederatedBenchmarking:
    ...
    def test_membership_churn_suppresses_benchmark_release(self) -> None:
        """High membership churn between periods suppresses publication."""
        self.fbp.create_cohort("churn-test")
        for i in range(5):
            org_id = f"org-{i}"
            self.fbp.join_cohort(org_id, "churn-test")
            self.fbp.submit_metric(
                BenchmarkMetric(
                    org_id=org_id,
                    metric_name="cost_per_token_usd",
        ...
        initial = self.fbp.publish_benchmark("churn-test", "cost_per_token_usd", "2026-Q1")
        ...
        for i in range(3, 8):
            ...
            self.fbp.submit_metric(
                BenchmarkMetric(
                    org_id=org_id,
                    metric_name="cost_per_token_usd",
                    raw_value=0.0035 + (i - 3) * 0.0001,
                    period="2026-Q2",
                )
            )

        suppressed = self.fbp.publish_benchmark("churn-test", "cost_per_token_usd", "2026-Q2")
        assert suppressed.suppressed is True
        assert suppressed.suppression_reason is not None
        assert "churn" in suppressed.suppression_reason.lower()
